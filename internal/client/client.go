...
type Client struct {
	...
	tunnelOrder []string // insertion order — drives snapshot() iteration
...
// removeFromOrder removes a tunnel ID from tunnelOrder.
// Caller must hold c.mu.
func (c *Client) removeFromOrder(id string) {
	for i, v := range c.tunnelOrder {
		if v == id {
			c.tunnelOrder = append(c.tunnelOrder[:i], c.tunnelOrder[i+1:]...)
			return
		}
	}
}
